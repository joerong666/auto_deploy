...
if [ $method = "init" ]; then
    ...
    for db_conf in "fooyun-web-beta/conf/application.conf" "fooyun-web-beta_cls/conf/application.conf" "fooyun-web-beta_cls2/conf/application.conf"
    do  
...
if [ $method = "rsync" ]; then
    ...
    rsync -av --exclude-from=$PWD/exclude.list $src_home/web/ fooyun-web-beta_cls2/
    ...
    rsync -av $src_home/dist/fooyun-dataserver-beta_x86_64.tar.gz fooyun-web-beta_cls2/public/deploy/fooyun-dataserver-beta_x86_64.tar.gz
    rsync -av $src_home/dist/fooyun-proxy-beta_x86_64.tar.gz fooyun-web-beta_cls2/public/deploy/fooyun-proxy-beta_x86_64.tar.gz

    #########################
    #Configurations
    #########################
    cnf_custom=(myconf/application.conf  myconf/application.conf_cls  myconf/application.conf_cls2  myconf/fooyun_mngr.ini  myconf/foo_conf.lua  myconf/mngr_config.lua)
    cnf_orig=(fooyun-web-beta/conf/application.conf  fooyun-web-beta_cls/conf/application.conf  fooyun-web-beta_cls2/conf/application.conf \
